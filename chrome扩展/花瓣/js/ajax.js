(function(){
  ...
  function constructBlobData(dataString, contentType) {
    ...
    // BlobBuilder was deprecated in webkit  http://trac.webkit.org/changeset/115666
    var bb = new Blob([data.buffer], {type: 'image/png'});
  ...
  function constructMultipartFormData(binaryData, boundary, otherParameters) {
    ...
    for (var key in otherParameters) {
      ...
      data.push('name="' + key + '"\r\n\r\n' + otherParameters[key] + '\r\n');
      fd.append(key, otherParameters[key]);
    }
    var len = binaryData.data.length;
    var data = new Uint8Array(len);
    for (var i = 0; i < len; i++) {
      data[i] = binaryData.data.charCodeAt(i);
    }
    // BlobBuilder was deprecated in webkit  http://trac.webkit.org/changeset/115666
    var bb = new Blob([data.buffer], {type: 'image/png'});
    fd.append((binaryData.name || 'binaryfilename'), bb, binaryData.value);
    return fd;
  ...
  ajax.constructBlobData = constructBlobData;
  window.ajax = ajax;
})();

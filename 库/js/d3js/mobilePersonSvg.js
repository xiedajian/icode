        ...
        .on("click", function (d) {
            if (d.link) {
                ...
                if (d.role_id) {
                    ...
                    window.location.href = "/tv/" + url + "/role/" + d.role_id + "/";
                }
            }
        });
...
var linksData = [];
// 隐藏更多关系、查看更多关系  汉字翻译
// var roleMapFlagMore = roleMapFlagMore;
// var roleMapFlagHide = roleMapFlagHide;
...
function initSVG(id) {
    ...
    if (nodesData.length <= (pageNum + 1)) {
        ...
        $("#" + id).append('<a href="###" class="showMoreData" data-container-id="' + id + '">' + roleMapFlagMore + '</a>');
    }
}
...
function initData(id, url,postData, sortField) {
    $.post(url,postData, function (data) {
        ...
        linksData.sort(function (a, b) {
            return (parseInt(a.target) + parseInt(a.source)) - (parseInt(b.target) + parseInt(b.source));
        });
...
//加载更多
$('body').on('tap', '.showMoreData', function () {
    ...
    $("#" + id).append('<a href="###" class="hideMoreData" data-container-id="' + id + '">' + roleMapFlagHide + '</a>');
});

//隐藏更多
$('body').on('tap', '.hideMoreData', function () {
    var id = $(this).attr('data-container-id');
    $("#" + id).empty();
    initSVG(id);
});
